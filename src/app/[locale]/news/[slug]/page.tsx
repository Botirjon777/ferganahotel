...
export async function generateMetadata({
  ...
  if (!item) {
    return {
      title: "News Article | Fergana Hotel",
      description: "Read the latest news and updates from Fergana Hotel.",
    };
  }
  ...
  if (item.id === "smart-in-room-service") {
    ...
    description =
      "Fergana Hotel elevates guest experience with the introduction of intelligent smart in-room service controls and digital assistant systems.";
  } else if (item.id === "quality-management-cert") {
    title = "ISO 9001:2015 Quality Management System Certification";
    description =
      "Fergana Hotel achieves the prestigious ISO 9001:2015 certification, reflecting our commitment to premium service quality and guest satisfaction.";
  } else if (item.id === "food-safety-cert") {
    title = "ISO 22000:2018 Food Safety Management Certification";
    description =
      "Fergana Hotel Gastrobar receives the ISO 22000:2018 certification, confirming our absolute adherence to elite international food safety and culinary hygiene standards.";
  } else {
    title = item.id
      .split("-")
      ...
      .join(" ");
    description =
      "Read this article to learn more about the latest news, announcements, and events at Fergana Hotel.";
  }
  ...
  return {
    ...
    openGraph: {
      type: "article",
      title: `${title} | Fergana Hotel`,
      description,
      url:
        locale === "uz"
          ? `https://ferganahotel.uz/news/${item.id}`
          : `https://ferganahotel.uz/${locale}/news/${item.id}`,
      publishedTime: new Date(item.date).toISOString(),
      authors: ["Fergana Hotel"],
